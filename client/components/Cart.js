...
import React, { Component } from 'react';
import { connect } from 'react-redux';
import { deleteFromCart, fetchCart, _updateCart, updateQuantity } from '../store/cart';
        ...
        {this.props.cart !== null && this.props.cart.products ? (
          this.props.cart.products.map((product) => (
            <div key={product.id} className="checkout-card-row">
              ...
              <div className='subtotal-inline-block' style={{marginRight: '5rem'}}>{product.cartItem.quantity}</div>
                  <button onClick={() => this.props.updateCart(product, -1)}>
                    {' '}
                    -{' '}
                  </button>
